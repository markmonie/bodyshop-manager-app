...
const EstimateApp = ({ userId }) => {
    ...
    if(mode==='DASHBOARD') return <div style={{padding:'20px'}}><button onClick={()=>setMode('ESTIMATE')}>Back</button><h2>Dashboard</h2><div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}><div style={{background:'#e0f2fe', padding:'10px'}}>Sales: £{sys.jobs.filter(j=>j.type?.includes('INV')).reduce((a,b)=>a+(b.totals?.finalDue||b.totals?.due||0),0).toFixed(0)}</div><div style={{background:'#dcfce7', padding:'10px'}}>Profit: £{(sys.jobs.filter(j=>j.type?.includes('INV')).reduce((a,b)=>a+(b.totals?.jobProfit||b.totals?.prof||0),0) - sys.exps.reduce((a,b)=>a+b.amount,0)).toFixed(0)}</div></div><h3>Expenses</h3><div style={{display:'flex'}}><input style={{flex:1}} placeholder="Desc" value={sys.expD} onChange={e=>setSys({...sys, expD:e.target.value})}/><input style={{width:60}} placeholder="£" value={sys.expA} onChange={e=>setSys({...sys, expA:e.target.value})}/><button onClick={actions.addExp}>+</button></div>{sys.exps.map(x=><div key={x.id} style={s.row}>{x.desc} <b>£{x.amount}</b></div>)}</div>;

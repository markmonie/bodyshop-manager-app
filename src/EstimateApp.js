    ...
    return (
        <div style={{ background: '#000', minHeight: '100vh', color: '#fff', padding: '20px', paddingBottom: '140px' }}>
                    ...
                    {history.map((h, i) => (
                        <div key={i} style={s.card('#444')}>
                            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                <div>
                                    <h3 style={{margin:0, color:theme.hub}}>{h.vehicle?.reg || 'NEW JOB'}</h3>
                                    <p style={{margin:0, fontSize:'12px', color:'#999'}}>{h.client?.name || 'No Name'} | {h.createdAt ? new Date(h.createdAt.seconds * 1000).toLocaleDateString() : 'Draft'}</p>
                                </div>
                                <div style={{display:'flex', gap:'10px'}}>
                                    <button style={{...s.btnG(theme.deal), padding:'12px 20px'}} onClick={()=>{setJob(h); setView('HUB');}}>LOAD</button>
                                    <button style={{...s.btnG(theme.danger), padding:'12px 20px'}} onClick={async () => {
                                        if(window.confirm(`Permanently Delete ${h.vehicle?.reg}?`)) {
                                            await deleteDoc(doc(db, 'estimates', h.id));
                                        }
                                    }}>X</button>
                                </div>
                            </div>
                ...
                <div>
                    ...
                    <div style={s.card(theme.fin)}>
                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'25px'}}>
                            ...
                            <div style={{background:'#000', padding:'25px', borderRadius:'15px'}}><span style={s.label}>Expenses</span><h2 style={{margin:0, color:theme.danger}}>{job.vault.expenses.length} Logged</h2></div>
                        </div>
                        <span style={s.label}>Upload Expense Receipt</span>
                        ...
                        <button style={{...s.btnG(theme.fin), width:'100%'}} onClick={() => {
                            let csv = "Date,Reg,Client,Total,Profit\n";
                            history.forEach(h => {
                                const d = h.createdAt ? new Date(h.createdAt.seconds * 1000).toLocaleDateString() : 'Draft';
                                csv += `${d},${h.vehicle?.reg},${h.client?.name},${h.totals?.total},${h.totals?.profit}\n`;
                            });
                            const blob = new Blob([csv], { type: 'text/csv' });
                            const url = window.URL.createObjectURL(blob);
                            const a = document.createElement('a'); a.href = url; a.download = 'TripleMMM_Revenue.csv'; a.click();
                        }}>EXPORT REVENUE CSV</button>
                    </div>
                </div>
            )}

            {/* ALL OTHER VIEWS (EST, WORK, SET) MAINTAINED IN FULL... */}
            {view === 'EST' && (
                <div>
                    <h2 style={{color:theme.hub}}>ESTIMATING: {job.vehicle.reg}</h2>
                    <div style={s.card(theme.hub)}>
                        <span style={s.label}>Repair Matrix</span>
            ...
            {view === 'SET' && (
                <div>
                    ...
                    <div style={s.card(theme.set)}>
                        ...
                        <textarea style={{...s.input, height:'120px', marginTop:'20px'}} value={settings.terms} onChange={e=>setSettings({...settings, terms:e.target.value})} />
                        <button style={{...s.btnG(theme.set), width:'100%', marginTop:'15px'}} onClick={async () => { await setDoc(doc(db, 'settings', 'global'), settings); alert("Settings Saved"); }}>SAVE MASTER CONFIG</button>
                    </div>
                </div>
            )}
            ...
            <div className="no-print" style={s.dock}>
                ...
                <button onClick={()=>setView('RECENT')} style={{...s.btnG(view === 'RECENT' ? theme.hub : '#222'), minWidth:'80px'}}>JOBS</button>
            ...
            {/* PRINT VIEW (RE-VERIFIED INVOICE) */}
            <div className="print-only" style={{display:'none', color:'black', padding:'60px', fontFamily:'Arial'}}>
                ...
                <div style={{textAlign:'right', marginTop:'45px'}}>
                    <h1 style={{color:'#f97316', fontSize:'48px'}}>TOTAL: £{totals.total.toFixed(2)}</h1>

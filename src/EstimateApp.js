...
const EstimateApp = ({ userId }) => {
    ...
    const resetJob = () => { if(window.confirm("⚠️ START NEW JOB?")) { localStorage.removeItem('mmm_v740_FINAL'); setJob(INITIAL_JOB); setClientMatch(null); window.scrollTo(0, 0); } };
                    ...
                    {docType === 'SATISFACTION NOTE' ? (
                        <div style={{marginTop:'60px', textAlign: 'center'}}>
                            ...
                            <div style={{display:'flex', flexDirection:'column', alignItems:'center'}}>
                                {job.vault.signature ? <img src={job.vault.signature} style={{width:'350px', borderBottom:'3px solid #000', paddingBottom:'5px'}} alt="Sig" /> : <div style={{width:'350px', height:'120px', borderBottom:'3px solid #000'}}></div>}
                        ...
                        <div>
                            <table style={{width:'100%', borderCollapse:'collapse', fontSize:'12px'}}>
                                ...
                                <tbody>
                                    ...
                                    <tr><td style={{padding:'8px'}}>Bodywork Labour ({totals.lHrs} hrs)</td><td style={{textAlign:'right', padding:'8px'}}>£{totals.lPrice.toFixed(2)}</td></tr>
                            ...
                            {docType === 'JOB CARD' ? (
                                <div style={{marginTop:'30px', border:'2px dashed #333', padding:'20px'}}><p style={{fontSize:'16px', whiteSpace:'pre-wrap'}}>{job.repair.techNotes || 'No notes.'}</p><div style={{marginTop:'50px', borderTop:'1px solid black', width:'200px'}}>Sign-off</div></div>
                            ) : (
                                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'30px'}}>
                                    <div style={{flex:1, textAlign:'center', border:'4px solid #f97316', borderRadius:'20px', padding:'20px', marginRight:'20px'}}><div style={{fontSize:'22px', fontWeight:'900'}}>BACS:<br/>{settings.bank}</div>{settings.paypalQr && <img src={settings.paypalQr} style={{height:'120px', marginTop:'10px'}} alt="QR" />}</div>
                                    <div style={{flex:1, textAlign:'right'}}><h1 style={{fontSize:'45px', margin:0, color:'#f97316'}}>£{printMode === 'EXCESS' ? parseFloat(job.repair.excess||0).toFixed(2) : printMode === 'INSURER' ? totals.insurer.toFixed(2) : totals.total.toFixed(2)}</h1></div>
                                </div>
                            )}

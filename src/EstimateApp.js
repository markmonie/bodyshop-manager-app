...
// NOTE: Since these are hardcoded, please ensure you have set up Domain Restrictions 
// in Google Cloud Console to prevent misuse if you make this public.
...
// This component was previously unused. It is now used in the SAT view.
...
const EstimateApp = ({ userId }) => {
    ...
    const resetJob = () => {
        if (window.confirm("⚠️ START NEW JOB?")) {
            localStorage.removeItem('mmm_v770_FINAL');
            setJob(INITIAL_JOB);
            setClientMatch(null);
            window.scrollTo(0, 0);
        }
    ...
    const runDVLA = async () => {
        if (!job?.vehicle?.reg || !settings.dvlaKey) { alert("Enter Reg & Key"); return; }
        setLoading(true);
        ...
        for (const proxy of proxies) {
            if (success) break;
            try {
                const response = await fetch(proxy + encodeURIComponent('https://driver-vehicle-licensing.api.gov.uk/vehicle-enquiry/v1/vehicles'), {
                    method: 'POST',
                ...
                if (response.ok) {
                    ...
                    if (data && data.make) {
                        setJob(prev => ({
                            ...prev,
                            vehicle: { ...prev.vehicle, make: data.make, year: data.yearOfManufacture, colour: data.colour, fuel: data.fuelType, engine: data.engineCapacity }
                        }));
                        success = true;
                    }
                }
    ...
    const downloadCSV = () => {
        ...
        link.setAttribute("download", `MMM_Ledger_${new Date().toLocaleDateString().replace(/\//g, '-')}.csv`);
                ...
                <div style={{ padding: '100px 40px 40px 40px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '8px solid #f97316', paddingBottom: '20px', marginBottom: '20px' }}>
                        <div style={{ flex: 1 }}>
                            ...
                            <p style={{ fontSize: '12px' }}>{settings.address}<br />{settings.phone}</p>
                        </div>
                        <div style={{ textAlign: 'right', flex: 1 }}>
                            ...
                            <div style={{ marginTop: '15px', fontSize: '12px', borderTop: '2px solid #ddd', paddingTop: '10px' }}>
                                <strong>TO:</strong> {printMode === 'INSURER' ? <>{job.insurance.name}<br />Ref: {job.insurance.claim}</> : <>{job.client.name}<br />{job.client.email}</>}
                            </div>
                        </div>
                    </div>
                    {docType === 'SATISFACTION NOTE' ? (
                        <div style={{ marginTop: '60px', textAlign: 'center' }}>
                            <h1 style={{ color: '#f97316', fontSize: '48px', marginBottom: '40px', fontWeight: '900' }}>SATISFACTION NOTE</h1>
                            <div style={{ border: '4px solid #000', padding: '30px', borderRadius: '20px', marginBottom: '50px', background: '#f4f4f5', display: 'inline-block', textAlign: 'left', minWidth: '550px' }}>
                                <table style={{ width: '100%', fontSize: '20px', fontWeight: 'bold' }}>
                                    <tbody>
                                        <tr><td style={{ padding: '12px' }}>REGISTRATION:</td><td style={{ padding: '12px' }}>{job.vehicle.reg}</td></tr>
                                        <tr><td style={{ padding: '12px' }}>INVOICE #:</td><td style={{ padding: '12px' }}>{job.invoiceNo || 'DRAFT'}</td></tr>
                                        <tr><td style={{ padding: '12px' }}>DATE:</td><td style={{ padding: '12px' }}>{job.invoiceDate || new Date().toLocaleDateString()}</td></tr>
                                    </tbody>
                                </table>
                            </div>
                            <p style={{ fontSize: '26px', lineHeight: '1.6', maxWidth: '750px', margin: '0 auto 60px auto', fontWeight: '500' }}>I, <strong>{job.client.name}</strong>, confirm repairs are completed to my satisfaction.</p>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                {job.vault.signature ? <img src={job.vault.signature} style={{ width: '350px', borderBottom: '3px solid #000' }} alt="Sig" /> : <div style={{ width: '350px', height: '120px', borderBottom: '3px solid #000' }}></div>}
                                <p style={{ marginTop: '15px', fontSize: '20px', fontWeight: '900' }}>Signature</p>
                            </div>
                        </div>
                    ) : (
                        <div>
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                                <thead><tr style={{ background: '#eee' }}><th style={{ padding: '8px', textAlign: 'left' }}>Description</th><th style={{ padding: '8px', textAlign: 'right' }}>Amount</th></tr></thead>
                                <tbody>
                                    {(job.repair.items || []).map((it, i) => (
                                        <tr key={i} style={{ borderBottom: '1px solid #eee' }}><td style={{ padding: '8px' }}>{it.desc}</td><td style={{ textAlign: 'right', padding: '8px' }}>£{(parseFloat(it.cost) * (1 + (parseFloat(settings.markup) / 100))).toFixed(2)}</td></tr>
                                    ))}
                                    <tr><td style={{ padding: '8px' }}>Labour ({totals.lHrs} hrs)</td><td style={{ textAlign: 'right', padding: '8px' }}>£{totals.lPrice.toFixed(2)}</td></tr>
                            ...
                            {docType === 'JOB CARD' ? (
                                <div style={{ marginTop: '30px', border: '2px dashed #333', padding: '20px' }}><p>{job.repair.techNotes || 'No notes.'}</p></div>
                            ) : (
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '30px' }}>
                                    <div style={{ flex: 1, textAlign: 'center', border: '4px solid #f97316', borderRadius: '20px', padding: '20px', marginRight: '20px' }}><div style={{ fontSize: '20px', fontWeight: '900' }}>BACS: {settings.bank}</div>{settings.paypalQr && <img src={settings.paypalQr} style={{ height: '120px', marginTop: '10px' }} alt="QR" />}</div>
                            ...
                            {docType !== 'JOB CARD' && settings.terms && <div style={{ pageBreakBefore: 'always', paddingTop: '20px' }}><h2 style={{ color: '#f97316', borderBottom: '4px solid #f97316', paddingBottom: '10px', fontSize: '18px', margin: 0 }}>TERMS</h2><div style={{ columnCount: 2, columnGap: '30px', fontSize: '10px', lineHeight: '1.4', marginTop: '15px', whiteSpace: 'pre-wrap' }}>{settings.terms}</div></div>}
                        </div>
                    )}
                </div>
            ...
            <div className="no-print">
                {view === 'HUB' && (
                    <div style={{ maxWidth: '850px', margin: '0 auto' }}>
                        <h1 style={{ color: theme.hub, fontSize: '65px', textAlign: 'center' }}>COMMAND HUB</h1>
                        <div style={s.card(theme.hub)}>
                            <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
                                <input style={{ ...s.input, flex: 4, fontSize: '55px', textAlign: 'center' }} value={job.vehicle.reg} onChange={e => setJob({ ...job, vehicle: { ...job.vehicle, reg: e.target.value.toUpperCase() } })} placeholder="REG" />
                                <button style={{ ...s.btnG(theme.hub), flex: 1 }} onClick={runDVLA}>FIND</button>
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                ...
                                <input style={s.input} value={job.vehicle.year} onChange={e => setJob({ ...job, vehicle: { ...job.vehicle, year: e.target.value } })} placeholder="SPEC" />
                            </div>
                        </div>
                        <div style={s.card(theme.deal)}>
                            ...
                            <input style={s.input} placeholder="Full Address" value={job.client.address} onChange={e => setJob({ ...job, client: { ...job.client, address: e.target.value } })} />
                        </div>
                        <button style={{ ...s.btnG(theme.deal), width: '100%', padding: '35px', fontSize: '32px' }} onClick={() => setView('EST')}>ESTIMATOR</button>
                        <button style={{ ...s.btnG(theme.danger), width: '100%', marginTop: '20px' }} onClick={resetJob}>⚠️ START NEW JOB</button>
                    </div>
                )}
                {view === 'EST' && (
                    <div style={{ maxWidth: '850px', margin: '0 auto' }}>
                        ...
                        <div style={s.card(theme.work)}>
                            ...
                            {job.repair.items.map((it, i) => (
                                <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                                    <input style={{ ...s.input, flex: 3 }} value={it.desc} placeholder="Part" onChange={(e) => { const n = [...job.repair.items]; n[i].desc = e.target.value; setJob({ ...job, repair: { ...job.repair, items: n } }); }} />
                                    <input style={{ ...s.input, flex: 1 }} value={it.cost} placeholder="£" onChange={(e) => { const n = [...job.repair.items]; n[i].cost = e.target.value; setJob({ ...job, repair: { ...job.repair, items: n } }); }} />
                                    <button onClick={() => { const n = job.repair.items.filter((_, idx) => idx !== i); setJob({ ...job, repair: { ...job.repair, items: n } }); }} style={{ ...s.btnG(theme.danger), padding: '15px' }}>X</button>
                                </div>
                            ))}
                ...
                {view === 'SAT' && (
                     <div style={{ maxWidth: '850px', margin: '0 auto' }}>
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '40px' }}><button style={{ ...s.btnG('#222'), flex: 1 }} onClick={() => setView('HUB')}>⬅️ BACK</button></div>
                        <div style={s.card(theme.deal)}>
                            <h2 style={{ color: theme.deal }}>SATISFACTION SIGN OFF</h2>
                            <p style={{marginBottom: '20px'}}>Please ask client to sign below.</p>
                            <NativeSignature onSave={(data) => setJob({...job, vault: {...job.vault, signature: data}})} />
                            <div style={{display:'flex', gap:'10px'}}>
                                <button style={s.btnG(theme.deal)} onClick={() => openDocument('SATISFACTION NOTE')}>GENERATE DOCUMENT</button>
                            </div>
                        </div>
                     </div>
                )}
                    ...
                    <div style={{ maxWidth: '850px', margin: '0 auto' }}>
                        ...
                        <div style={s.card(theme.fin)}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px', marginBottom: '40px' }}>
                                <div style={s.displayBox}><span style={s.label}>REVENUE</span><div style={{ fontSize: '40px' }}>£{history.reduce((a, b) => a + (b.totals?.total || 0), 0).toFixed(0)}</div></div>
                                <div style={s.displayBox}><span style={s.label}>RECEIPTS</span><div style={{ fontSize: '40px', color: theme.danger }}>{job.vault?.expenses?.length || 0}</div></div>
                            </div>
                ...
                {view === 'RECENT' && (
                    <div style={{ maxWidth: '850px', margin: '0 auto' }}>
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '40px' }}><button style={{ ...s.btnG('#222'), flex: 1 }} onClick={() => setView('HUB')}>⬅️ BACK</button></div>
                        <input style={s.input} placeholder="Search jobs..." value={vaultSearch} onChange={e => setVaultSearch(e.target.value)} />
                        {history.filter(h => JSON.stringify(h).toLowerCase().includes(vaultSearch.toLowerCase())).map((h) => (
                            <div key={h.id} style={{ ...s.card('#333'), display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '25px' }}>
                                <div><h2 style={{ margin: 0, color: theme.hub }}>{h.vehicle?.reg}</h2><p style={{ margin: 0 }}>{h.client?.name}</p></div>
                                <div style={{ display: 'flex', gap: '10px' }}><button style={s.btnG(theme.deal)} onClick={() => loadJob(h)}>OPEN</button><button style={s.btnG(theme.danger)} onClick={() => deleteJob(h.id)}>DEL</button></div>
                            </div>
                        ))}
                    </div>
                )}
                    ...
                    <div style={{ maxWidth: '850px', margin: '0 auto' }}>
                        ...
                        <div style={s.card(theme.set)}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                                ...
                                <div><span style={s.label}>Labour £/h</span><input style={s.input} value={settings.labourRate} onChange={e => setSettings({ ...settings, labourRate: e.target.value })} /></div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                                <button style={s.btnG('#222')} onClick={() => document.getElementById('logoIn').click()}>Logo</button>
                                <button style={s.btnG('#222')} onClick={() => document.getElementById('qrIn').click()}>QR</button>

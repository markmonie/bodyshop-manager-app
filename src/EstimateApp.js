...
const NativeSignature = ({ onSave }) => {
    ...
    const getXY = (e) => { if (!canvasRef.current) return { x: 0, y: 0 }; const rect = canvasRef.current.getBoundingClientRect(); let cX, cY; if (e.changedTouches && e.changedTouches.length > 0) { cX = e.changedTouches[0].clientX; cY = e.changedTouches[0].clientY; } else { cX = e.clientX; cY = e.clientY; } return { x: cX - rect.left, y: cY - rect.top }; };
...
const EstimateApp = ({ userId }) => {
    ...
    const totals = useMemo(() => {
        ...
        const excessAmt = n(job.repair.excess);
        const insurerPays = total - excessAmt;
        return { total, insurer: insurerPays, lHrs, lPrice, partsRaw, net: subtotal, vat: vatAmount, excess: excessAmt };
    }, [job.repair, settings]);
    ...
    const downloadIncomeCSV = () => {
        const headers = ["Status", "Date", "Invoice No", "Reg", "Payer", "Net Income", "VAT", "Total Billed"];
        const rows = history.flatMap(h => {
            const snap = h.totals || {};
            const vatR = parseFloat(settings.vatRate || 20) / 100;
            const grossTotal = snap.total || 0;
            const excess = parseFloat(h.repair?.excess || 0);
            const status = h.invoiceNo ? "FINAL" : "ESTIMATE";
            const displayDate = h.invoiceDate || new Date(h.createdAt?.seconds * 1000).toLocaleDateString();
            const invNo = h.invoiceNo || 'DRAFT';
            const reg = h.vehicle?.reg || 'N/A';
            const output = [];
            const mainBillTotal = grossTotal - excess;
            const mainBillNet = mainBillTotal / (1 + vatR);
            const mainBillVat = mainBillTotal - mainBillNet;
            const payerName = excess > 0 ? (h.insurance?.name || "INSURANCE") : (h.client?.name || "CLIENT");
            output.push([status, displayDate, invNo, reg, `"${payerName}"`, mainBillNet.toFixed(2), mainBillVat.toFixed(2), mainBillTotal.toFixed(2)]);
            if (excess > 0) {
                const excessNet = excess / (1 + vatR);
                const excessVat = excess - excessNet;
                output.push([status, displayDate, invNo + "-XS", reg, `"${h.client?.name || 'CLIENT'} (EXCESS)"`, excessNet.toFixed(2), excessVat.toFixed(2), excess.toFixed(2)]);
            }
            return output;
        });
    ...
    const openDocument = async (type, mode = 'FULL') => {
        setDocType(type); setPrintMode(mode); let curInv = job.invoiceNo; 
        const docDate = job.invoiceDate || new Date().toLocaleDateString('en-GB');
        if (type === 'INVOICE' && !curInv) {
            const nextNum = (parseInt(settings.invoiceCount || 1000) + 1).toString();
            setJob(prev => ({ ...prev, invoiceNo: nextNum, invoiceDate: docDate }));
    ...
    const handleFileUpload = async (e, category, fieldName = null) => { 
        ...
        setLoading(true); 
        try { 
            const r = ref(storage, `${category}/${Date.now()}_${file.name}`); 
    ...
    if (view === 'PREVIEW') {
        return (
            <div style={{background:'#fff', minHeight:'100vh', color:'#000', fontFamily:'Arial'}}>
                <div className="no-print" style={{position:'fixed', top:0, left:0, right:0, background:theme.deal, padding:'20px', zIndex:9999, display:'flex', gap:'10px', boxShadow:'0 4px 12px rgba(0,0,0,0.3)'}}><button style={{...s.btnG('#333'), width:'100%', fontSize:'20px', fontWeight:'900'}} onClick={() => { setView(docType === 'SATISFACTION NOTE' ? 'SAT' : 'EST'); document.title="Triple MMM"; }}>⬅️ BACK TO APP</button></div>
                            ...
                            {printMode === 'EXCESS' ? (
                                <div style={{marginTop:'40px', fontSize:'18px'}}>
                                    <table style={{width:'100%', borderCollapse:'collapse'}}>
                                        <thead><tr style={{borderBottom:'2px solid #ddd'}}><th style={{textAlign:'left', padding:'10px'}}>Description</th><th style={{textAlign:'right', padding:'10px'}}>Amount</th></tr></thead>
                                        <tbody>
                                            <tr><td style={{padding:'15px'}}>Insurance Policy Excess (Reg: {job.vehicle.reg})</td><td style={{textAlign:'right', padding:'15px'}}>£{parseFloat(job.repair.excess || 0).toFixed(2)}</td></tr>
                                            <tr style={{borderTop:'2px solid #000'}}><td style={{fontWeight:'bold', padding:'15px'}}>TOTAL DUE:</td><td style={{textAlign:'right', fontWeight:'bold', padding:'15px', fontSize:'22px'}}>£{parseFloat(job.repair.excess || 0).toFixed(2)}</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                            ) : (
                                <table style={{width:'100%', borderCollapse:'collapse', fontSize:'12px'}}>
                                    ...
                                    <tbody>
                                        {(job.repair.items || []).map((it, i) => (<tr key={i} style={{borderBottom:'1px solid #eee'}}><td style={{padding:'8px'}}>{it.desc}</td><td style={{textAlign:'right', padding:'8px'}}>£{(parseFloat(it.cost)*(1+(parseFloat(settings.markup)/100))).toFixed(2)}</td></tr>))}
                                        ...
                                        {printMode === 'INSURER' && parseFloat(job.repair.excess) > 0 && (
                                            <>
                                                ...
                                                <tr style={{borderTop:'4px solid #000'}}><td style={{padding:'8px', textAlign:'right', fontWeight:'bold', fontSize:'18px'}}>AMOUNT DUE FROM INSURER:</td><td style={{textAlign:'right', padding:'8px', fontWeight:'bold', fontSize:'18px'}}>£{totals.insurer.toFixed(2)}</td></tr>
                                            </>
                                        )}
                                    </tbody>
                                </table>
                            )}
                            {docType === 'JOB CARD' && (<div style={{marginTop:'30px', border:'2px dashed #333', padding:'20px'}}><p>{job.repair.techNotes || 'No notes.'}</p></div>)}
                            {docType !== 'JOB CARD' ? (<div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'30px'}}><div style={{flex:1, textAlign:'center', border:'4px solid #f97316', borderRadius:'20px', padding:'20px', marginRight:'20px'}}><div style={{fontSize:'20px', fontWeight:'900'}}>BACS: {settings.bank}</div>{settings.paypalQr && <img src={settings.paypalQr} style={{height:'120px', marginTop:'10px'}} alt="QR" />}</div><div style={{flex:1, textAlign:'right'}}><h1 style={{fontSize:'45px', margin:0, color:'#f97316'}}>£{printMode === 'EXCESS' ? parseFloat(job.repair.excess||0).toFixed(2) : printMode === 'INSURER' ? totals.insurer.toFixed(2) : totals.total.toFixed(2)}</h1></div></div>) : null}
                        </div>
                    )}
                </div>
    ...
    return (
        <div style={{ background: '#000', minHeight: '100vh', color: '#fff', padding: '20px', paddingBottom: '180px' }}>
            ...
            <div className="no-print">
                {view === 'HUB' && (
                    <div style={{maxWidth:'850px', margin:'0 auto'}}><h1 style={{color:theme.hub, fontSize:'65px', textAlign:'center'}}>COMMAND HUB</h1>
                        ...
                        <div style={s.card('#888')}>
                            ...
                            <input style={s.input} placeholder="Insurance Company" value={job.insurance.name} onChange={e=>setJob({...job, insurance:{...job.insurance, name:e.target.value}})} />
                            ...
                            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px'}}>
                                <input style={s.input} placeholder="Network" value={job.insurance.network} onChange={e=>setJob({...job, insurance:{...job.insurance, network:e.target.value}})} />
                                <input style={s.input} placeholder="Excess (£)" value={job.repair.excess} onChange={e=>setJob({...job, repair:{...job.repair, excess:e.target.value}})} />
                            </div>
                            ...
                            <input style={s.input} placeholder="Insurance Address" value={job.insurance.address} onChange={e=>setJob({...job, insurance:{...job.insurance, address:e.target.value}})} />
                        </div>
                        <button style={{...s.btnG(theme.deal), width:'100%', padding:'35px', fontSize:'32px'}} onClick={()=>setView('EST')}>ESTIMATOR</button><button style={{...s.btnG(theme.danger), width:'100%', marginTop:'20px'}} onClick={resetJob}>⚠️ START NEW JOB</button>
                    </div>
                )}
                {view === 'EST' && (
                    ...
                    <div style={s.card(theme.deal)}>
                        <div style={{display:'flex', justifyContent:'space-between', fontSize:'24px', fontWeight:'bold', borderBottom:'1px solid #333', paddingBottom:'10px', marginBottom:'10px'}}><span>Total Job:</span><span>£{totals.total.toFixed(2)}</span></div>
                ...
                {view === 'FIN' && (
                    ...
                    {/* V890: DIGITAL INBOX */}
                    <div style={{border:'2px dashed #444', padding:'20px', borderRadius:'15px', marginTop:'20px'}}>
                        <span style={s.label}>INVOICE DROP ZONE (For Ghost Scrubber)</span>
                        <input type="file" accept="application/pdf,image/*" onChange={(e) => handleFileUpload(e, 'invoices')} style={{marginTop:'10px', marginBottom:'20px'}} />
                        <div style={{display:'flex', gap:'10px', overflowX:'auto'}}>
                            {(job.vault?.supplierInvoices || []).map((url, i) => (
                                <a key={i} href={url} target="_blank" rel="noopener noreferrer" style={{display:'block', width:'100px', height:'100px', background:'#333', borderRadius:'10px', color:'#fff', textDecoration:'none', display:'flex', alignItems:'center', justifyContent:'center', border:'2px solid #555'}}>PDF {i+1}</a>
                            ))}
                        </div>
                    </div>
                    ...
                    <div style={{marginTop:'30px'}}>
                        ...
                        <span style={s.label}>Small Receipts</span><input type="file" onChange={(e) => handleFileUpload(e, 'expenses')} style={{marginBottom:'20px'}} />
                        <div style={{display:'flex', gap:'10px', overflowX:'auto'}}>{(job.vault?.expenses || []).map((url, i) => <img key={i} src={url} style={{height:'100px', border:'2px solid #333', borderRadius:'10px'}} alt="Receipt" />)}</div>
                    </div></div></div>
                )}
                {view === 'RECENT' && (

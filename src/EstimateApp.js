...
const EstimateApp = ({ userId }) => {
    ...
    const [settings, setSettings] = useState({ 
        ...
        dvlaKey: 'IXqv1yDD1latEPHIntk2w8MEuz9X57IE9TP9sxGc', logoUrl: '', paypalQr: '', password: '1234', terms: 'Standard 12-Month Bodywork Warranty. Payment due strictly on collection.' 
    });
    ...
    useEffect(() => {
        ...
        const saved = localStorage.getItem('mmm_enterprise_v31_LOCKED');
        if (saved) setJob(JSON.parse(saved));
        onSnapshot(query(collection(db, 'estimates'), orderBy('createdAt', 'desc')), snap => setHistory(snap.docs.map(d => ({id:d.id, ...d.data()}))));
    }, []);

    useEffect(() => { localStorage.setItem('mmm_enterprise_v31_LOCKED', JSON.stringify(job)); }, [job]);
    ...
    const runDVLA = async () => {
        ...
        } catch (e) { alert("DVLA Handshake Error. Manual Entry Mode."); }
        setLoading(false);
    };
            ...
            {view === 'DEAL' && (
                <div>
                    ...
                    <div style={s.card(theme.deal)}>
                        <span style={s.label}>Snapshot History</span>
                        ...
                        <input type="file" multiple onChange={(e) => handleFileUpload(e, 'claims', 'photos')} style={{marginBottom:'15px'}} />
                ...
                <div>
                    ...
                    <div style={s.card(theme.fin)}>
                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'25px'}}>
                            ...
                            <div style={{background:'#000', padding:'25px', borderRadius:'15px'}}><span style={s.label}>Expenses</span><h2 style={{margin:0, color:theme.danger}}>{job.vault.expenses.length}</h2></div>
                        </div>
            ...
            {view === 'SET' && (
                <div>
                    ...
                    <div style={s.card(theme.set)}>
                        ...
                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', background:'#000', padding:'25px', borderRadius:'15px', border:'1px solid #333', marginBottom:'20px'}}>
                            <div><span style={s.label}>Business Logo</span>{settings.logoUrl && <img src={settings.logoUrl} style={{height:'40px', marginBottom:'10px', display:'block'}} />}<input type="file" onChange={(e) => handleFileUpload(e, 'branding', 'logoUrl')} style={{fontSize:'11px'}} /></div>
                            <div><span style={s.label}>PayPal QR</span>{settings.paypalQr && <img src={settings.paypalQr} style={{height:'40px', marginBottom:'10px', display:'block'}} />}<input type="file" onChange={(e) => handleFileUpload(e, 'branding', 'paypalQr')} style={{fontSize:'11px'}} /></div>
                        </div>
                        <input style={s.input} placeholder="Company Name" value={settings.coName} onChange={e=>setSettings({...settings, coName:e.target.value})} />
                        <input style={s.input} placeholder="Sort/Acc" value={settings.bank} onChange={e=>setSettings({...settings, bank:e.target.value})} />
                        <textarea style={{...s.input, height:'100px'}} value={settings.terms} onChange={e=>setSettings({...settings, terms:e.target.value})} />
                        <button style={{...s.btnG(theme.set), width:'100%'}} onClick={async () => { await setDoc(doc(db, 'settings', 'global'), settings); alert("Master Confirmed."); }}>SAVE SETTINGS (GREEN)</button>
                    </div>
                </div>
            )}
            ...
            <div className="print-only" style={{display:'none', color:'black', padding:'60px', fontFamily:'Arial'}}>
                <div style={{display:'flex', justifyContent:'space-between', borderBottom:'6px solid #f97316', paddingBottom:'35px'}}>
                    ...
                    <div style={{textAlign:'right'}}>
                        <h2 style={{color:'#f97316', fontSize:'48px', margin:0, fontWeight:'900'}}>{docType}</h2>
                ...
                <div style={{minHeight:'60px', border:'1px solid #eee', padding:'15px', marginTop:'10px', borderRadius:'10px', fontSize:'14px', fontStyle:'italic'}}>{job.repair.techNotes || 'Repair carried out to professional standard.'}</div>
                ...
                <table style={{width:'100%', marginTop:'30px', borderCollapse:'collapse'}}>
                    ...
                    <tbody>
                        ...
                        <tr style={{borderBottom:'1px solid #eee'}}><td style={{padding:'18px'}}>Bodywork Labour Preparation ({totals.labHrs} hrs)</td><td style={{textAlign:'right', padding:'18px', fontWeight:'bold'}}>£{totals.labPrice.toFixed(2)}</td></tr>
                    </tbody>
                </table>

                <div style={{display:'flex', justifyContent:'space-between', marginTop:'40px'}}>
                    <div>{docType === 'INVOICE' && settings.paypalQr && <img src={settings.paypalQr} style={{height:'140px'}} />}</div>
                    <div style={{width:'55%', textAlign:'right'}}>
                        <h1 style={{color:'#f97316', fontSize:'52px', margin:'0 0 20px 0'}}>£{totals.total.toFixed(2)}</h1>
                        <div style={{background:'#fff3e0', padding:'30px', border:'2px solid #f97316', borderRadius:'25px', textAlign:'left'}}>
                            <div style={{display:'flex', justifyContent:'space-between', marginBottom:'12px', fontSize:'15px'}}><span>CUSTOMER EXCESS:</span><strong style={{fontSize:'22px'}}>£{totals.customer.toFixed(2)}</strong></div>

...
const styles = { inp: {width:'100%',padding:'8px',marginBottom:'8px',border:'1px solid #ccc',borderRadius:'4px'}, head: {borderBottom:'2px solid #c00',paddingBottom:'5px',marginBottom:'10px',color:'#c00'}, row: {display:'flex',justifyContent:'space-between',padding:'2px 0'}, btn: {padding:'10px',background:'#333',color:'#fff',border:'none',borderRadius:'4px',cursor:'pointer'}, stage: {padding:'10px',borderRadius:'6px',border:'1px solid #ccc',marginBottom:'10px',cursor:'pointer',display:'flex',justifyContent:'space-between',background:'#fff'} };
...
const EstimateApp = ({ userId }) => {
    ...
    // SAFE DRAFT LOADER (Prevents Crash)
    useEffect(() => { 
        try {
            const d = localStorage.getItem('draft_v2'); 
            if(d) { 
    ...
    useEffect(() => { if(mode==='ESTIMATE') localStorage.setItem('draft_v2', JSON.stringify({ c:cust, v:veh, i:item, f:fin, ph:sys.photos })); }, [cust, veh, item, fin, sys.photos, mode]);
    ...
    const actions = {
        load: (j) => { 
            // SAFETY LOAD: Checks if data exists before setting
        ...
        save: async (type) => { 
            ...
            try {
                ...
                const data = { type: t, status: 'UNPAID', invoiceNumber: n, customer: cust.n, address: cust.a, phone: cust.p, email: cust.e, claimNum: cust.c, networkCode: cust.nc, insuranceCo: cust.ic, insuranceAddr: cust.ia, reg: veh.r, mileage: veh.m, makeModel: veh.mm, vin: veh.v, paintCode: veh.pc, items: item.list, laborHours: fin.lh, laborRate: fin.lr, vatRate: fin.vat, excess: fin.ex, photos: sys.photos, bookingDate: veh.bd, bookingTime: veh.bt, totals: calc(), createdAt: serverTimestamp(), createdBy: userId, dealFile: { meth: false }, stages: {}, notes: [], hasFlag: false, paintAllocated: fin.paint };
    ...
    const Stage = ({k, l}) => { const x = sys.stages[k]||{}; return <div style={{...styles.stage, background:x.completed?'#f0fdf4':'#fff', borderColor:x.completed?'green':'#ccc'}}><div style={{display:'flex', gap:'10px'}}><input type="checkbox" checked={x.completed||false} onChange={e=>actions.stage(k,e.target.checked)}/><b>{l}</b></div>{x.completed && <small>{x.tech}<br/>{x.hours}h</small>}</div> };

    if(mode==='SETTINGS') return <div style={{padding:'20px'}}><button onClick={()=>setMode('ESTIMATE')}>Back</button><h2>Settings</h2>{Object.keys(cfg).map(k=><div key={k} style={{marginBottom:'10px'}}><label>{k}</label><input style={styles.inp} value={cfg[k]} onChange={e=>setCfg({...cfg, [k]:e.target.value})}/></div>)}<button style={styles.btn} onClick={()=>setDoc(doc(db,'settings','global'), cfg)}>Save</button></div>;
    
    if(mode==='DASHBOARD') return <div style={{padding:'20px'}}><button onClick={()=>setMode('ESTIMATE')}>Back</button><h2>Dashboard</h2><div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}><div style={{background:'#e0f2fe', padding:'10px'}}>Sales: £{sys.jobs.filter(j=>j.type?.includes('INV')).reduce((a,b)=>a+(b.totals?.finalDue||b.totals?.due||0),0).toFixed(0)}</div><div style={{background:'#dcfce7', padding:'10px'}}>Profit: £{(sys.jobs.filter(j=>j.type?.includes('INV')).reduce((a,b)=>a+(b.totals?.jobProfit||b.totals?.prof||0),0) - sys.exps.reduce((a,b)=>a+b.amount,0)).toFixed(0)}</div></div><h3>Expenses</h3><div style={{display:'flex'}}><input style={{flex:1}} placeholder="Desc" value={sys.expD} onChange={e=>setSys({...sys, expD:e.target.value})}/><input style={{width:60}} placeholder="£" value={sys.expA} onChange={e=>setSys({...sys, expA:e.target.value})}/><button onClick={actions.addExp}>+</button></div>{sys.exps.map(x=><div key={x.id} style={styles.row}>{x.desc} <b>£{x.amount}</b></div>)}</div>;
    ...
    return (
        <div style={{padding:'20px', maxWidth:'800px', margin:'0 auto', fontFamily:'sans-serif'}}>
            ...
            {mode!=='DEAL_FILE' && mode!=='JOBCARD' && <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px'}}>
                <div>
                    <h4 style={styles.head}>CLIENT</h4>
                    <input style={styles.inp} placeholder="Name" value={cust.n} onChange={e=>setCust({...cust, n:e.target.value})}/>
                    <textarea style={{...styles.inp, height:60}} placeholder="Address" value={cust.a} onChange={e=>setCust({...cust, a:e.target.value})}/>
                    <input style={styles.inp} placeholder="Phone" value={cust.p} onChange={e=>setCust({...cust, p:e.target.value})}/>
                    <input style={styles.inp} placeholder="Email" value={cust.e} onChange={e=>setCust({...cust, e:e.target.value})}/>
                    <div style={{display:'flex', gap:5}}><input style={styles.inp} placeholder="Claim" value={cust.c} onChange={e=>setCust({...cust, c:e.target.value})}/><input style={styles.inp} placeholder="Network" value={cust.nc} onChange={e=>setCust({...cust, nc:e.target.value})}/></div>
                    {fin.ex > 0 && <input style={{...styles.inp, borderColor:'orange'}} placeholder="Insurer" value={cust.ic} onChange={e=>setCust({...cust, ic:e.target.value})}/>}
                </div>
                <div>
                    <h4 style={styles.head}>VEHICLE</h4>
                    <div style={{display:'flex', gap:5}}><input style={{...styles.inp, background:'#e0f2fe', fontWeight:'bold'}} placeholder="REG" value={veh.r} onChange={e=>setVeh({...veh, r:e.target.value.toUpperCase()})} onBlur={()=>actions.lookup()}/></div>
                    <input style={styles.inp} placeholder="Model" value={veh.mm} onChange={e=>setVeh({...veh, mm:e.target.value})}/>
                    <div style={{display:'flex', gap:5}}><input style={styles.inp} placeholder="VIN" value={veh.v} onChange={e=>setVeh({...veh, v:e.target.value})}/><button className="no-print" onClick={()=>window.open(`https://7zap.com/en/search/?q=${veh.v}`)}>Parts</button></div>
                    <div style={{display:'flex', gap:5}}><input style={styles.inp} type="date" value={veh.bd} onChange={e=>setVeh({...veh, bd:e.target.value})}/><button className="no-print" onClick={actions.cal}>Cal</button></div>
            ...
            {mode!=='SATISFACTION' && mode!=='DEAL_FILE' && mode!=='JOBCARD' && <>
                ...
                <table style={{width:'100%', marginBottom:20}}><thead><tr style={{borderBottom:'2px solid #000', textAlign:'left'}}><th>Desc</th><th style={{textAlign:'right'}}>Price</th></tr></thead><tbody>
                    {invType==='EXCESS' ? <tr><td>Excess: {cust.c}</td><td style={{textAlign:'right'}}>£{fin.ex}</td></tr> : item.list.map((i,x)=><tr key={x} style={{borderBottom:'1px solid #eee'}}><td>{i.desc}</td><td style={{textAlign:'right'}}>£{(i.p||0).toFixed(2)} <span className="no-print" onClick={()=>actions.delItem(x)} style={{color:'red', cursor:'pointer'}}>x</span></td></tr>)}
                </tbody></table>
            ...
            <div className="no-print" style={{position:'fixed', bottom:0, left:0, right:0, background:'white', padding:10, borderTop:'1px solid #ccc', display:'flex', gap:5, overflowX:'auto', justifyContent:'center'}}>
                <button onClick={()=>actions.save('ESTIMATE')} style={{...styles.btn, background:sys.save==='SUCCESS'?'green':'#333'}}>{sys.save==='SAVING'?'...':'SAVE'}</button>
                {mode==='ESTIMATE' && <>{fin.ex>0 ? <><button onClick={()=>actions.save('INVOICE_MAIN')} style={{...styles.btn, background:'blue'}}>INS</button><button onClick={()=>actions.save('INVOICE_EXCESS')} style={{...styles.btn, background:'orange'}}>CUST</button></> : <button onClick={()=>actions.save('INVOICE')} style={styles.btn}>INV</button>}</>}
                <button onClick={()=>setMode('JOBCARD')} style={styles.btn}>JOB</button><button onClick={()=>setMode('DEAL_FILE')} style={styles.btn}>FILE</button><button onClick={actions.newJob} style={{...styles.btn, background:'red'}}>NEW</button><button onClick={()=>setMode('SETTINGS')} style={styles.btn}>⚙️</button><button onClick={()=>setMode('DASHBOARD')} style={styles.btn}>📊</button><button onClick={handlePrint} style={styles.btn}>🖨️</button>
            </div>

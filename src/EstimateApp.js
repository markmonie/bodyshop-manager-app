...
// --- NATIVE SIGNATURE ENGINE (PREVENTS BUILD FAILURES) ---
const NativeSignature = ({ onSave }) => {
    ...
    return (
        <div style={{ background: '#fff', borderRadius: '15px', padding: '10px' }}>
            <canvas ref={canvasRef} width={400} height={200} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={()=>setIsDrawing(false)} onTouchStart={startDrawing} onTouchMove={draw} onTouchEnd={()=>{setIsDrawing(false); onSave(canvasRef.current.toDataURL())}} style={{ width: '100%', height: '180px', cursor: 'crosshair', touchAction: 'none' }} />
...
const EstimateApp = ({ userId }) => {
    ...
    useEffect(() => {
        ...
        const saved = localStorage.getItem('mmm_final_v43_STABLE');
        if (saved) setJob(JSON.parse(saved));
        onSnapshot(query(collection(db, 'estimates'), orderBy('createdAt', 'desc')), snap => setHistory(snap.docs.map(d => ({id:d.id, ...d.data()}))));
    }, []);

    useEffect(() => { localStorage.setItem('mmm_final_v43_STABLE', JSON.stringify(job)); }, [job]);
    ...
    const saveMaster = async () => {
        ...
        alert("Triple MMM Synced: Snapshot archived in Deal Folder.");
    };

    const sendEmail = () => {
        const subject = `Repair Claim Submission: ${job.vehicle.reg} - ${job.client.name}`;
        const body = `Claims Department,\n\nRepair Documentation for ${job.vehicle.reg}.\nInsurer Balance: £${totals.insurer.toFixed(2)}\nClient Excess: £${totals.customer.toFixed(2)}\n\nFiles archived in Deal Folder.`;
            ...
            <div className="no-print">
                ...
                {view === 'HUB' && (
                    <div>
                        ...
                        <div style={s.card(theme.hub)}>
                            <span style={s.label}>2. Customer Meta</span>
                ...
                {view === 'EST' && (
                    <div>
                        ...
                        <div style={s.card(theme.hub)}>
                            <span style={s.label}>1. Labour Breakdown (MET/Panel/Paint)</span>
                            ...
                            <span style={s.label}>2. Parts Matrix (RESTORED)</span>
                            ...
                            <span style={s.label}>3. Financial Split</span>
                ...
                {view === 'WORK' && (
                    <div>
                        ...
                        <div style={s.card(theme.work)}>
                            ...
                            <textarea style={{...s.input, height:'180px'}} value={job.repair.techNotes} onChange={e=>setJob({...job, repair:{...job.repair, techNotes:e.target.value}})} placeholder="Log damage findings..." />
                        </div>
                    </div>
                )}

                {/* DEAL FOLDER (ARCHIVE RESTORED) */}
                {view === 'DEAL' && (
                    <div>
                        ...
                        <div style={s.card(theme.deal)}>
                            ...
                            <span style={s.label} style={{marginTop:'25px'}}>2. Archived Snapshots (CSR ARCHIVE)</span>
                            ...
                            <button style={{...s.btnG(theme.fin), width:'100%', marginBottom:'20px'}} onClick={sendEmail}>SEND CLAIM PACKAGE (EMAIL)</button>
                ...
                {view === 'CAL' && (
                    <div>
                        ...
                        <div style={s.card(theme.set)}>
                            <span style={s.label}>Schedule In (markmonie72@gmail.com)</span>
                    ...
                    <div>
                        ...
                        <div style={s.card(theme.fin)}>
                            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'25px'}}>
                                ...
                                <div style={{background:'#000', padding:'25px', borderRadius:'15px'}}><span style={s.label}>Receipts</span><h2 style={{margin:0, color:theme.danger}}>{job.vault.expenses?.length || 0}</h2></div>
                            </div>
                ...
                {view === 'SET' && (
                    <div>
                        ...
                        <div style={s.card(theme.set)}>
                            ...
                            <input style={s.input} placeholder="Calendar Email" value={settings.calId} onChange={e=>setSettings({...settings, calId:e.target.value})} />

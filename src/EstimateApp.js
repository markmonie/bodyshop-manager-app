...
const LoadingOverlay = () => (
    <div style={s.loader}><div style={{border: '5px solid #333', borderTop: `5px solid ${theme.hub}`, borderRadius: '50%', width: '60px', height: '60px', animation: 'spin 1s linear infinite', marginBottom:'20px'}}></div>DEPLOYING...<style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style></div>
);
...
const EstimateApp = ({ userId }) => {
    ...
    const resetJob = () => { if(window.confirm("⚠️ START NEW JOB?")) { localStorage.removeItem('mmm_v760_FINAL'); setJob(INITIAL_JOB); setClientMatch(null); window.scrollTo(0, 0); } };
    ...
    const handleFileUpload = async (e, path, field) => { const file = e.target.files[0]; if (!file) return; setLoading(true); try { const r = ref(storage, `${path}/${Date.now()}_${file.name}`); await uploadBytes(r, file); const url = await getDownloadURL(r); if (path === 'branding') setSettings(prev => ({...prev, [field]: url})); else setJob(prev => ({...prev, vault: {...prev.vault, expenses: [...(prev.vault.expenses || []), url]}})); } catch(err) { console.log(err); alert("Upload error"); } setLoading(false); };

    // --- RENDER ---
    if (view === 'PREVIEW') {
        return (
            <div style={{background:'#fff', minHeight:'100vh', color:'#000', fontFamily:'Arial'}}>
                <div className="no-print" style={{position:'fixed', top:0, left:0, right:0, background:theme.deal, padding:'20px', zIndex:9999, display:'flex', gap:'10px'}}><button style={{...s.btnG('#333'), width:'100%', fontSize:'20px', fontWeight:'900'}} onClick={() => { setView(docType === 'SATISFACTION NOTE' ? 'SAT' : 'EST'); document.title="Triple MMM"; }}>⬅️ BACK TO APP</button></div>
                <div style={{padding:'100px 40px 40px 40px'}}>
                    ...
                    {docType === 'SATISFACTION NOTE' ? (
                        ...
                        <div><table style={{width:'100%', borderCollapse:'collapse', fontSize:'12px'}}><thead><tr style={{background:'#eee', borderBottom:'2px solid #ddd'}}><th style={{padding:'8px', textAlign:'left'}}>Description</th><th style={{padding:'8px', textAlign:'right'}}>Amount</th></tr></thead><tbody>{(job.repair.items || []).map((it, i) => (<tr key={i} style={{borderBottom:'1px solid #eee'}}><td style={{padding:'8px'}}>{it.desc}</td><td style={{textAlign:'right', padding:'8px'}}>£{(parseFloat(it.cost)*(1+(parseFloat(settings.markup)/100))).toFixed(2)}</td></tr>))}<tr><td style={{padding:'8px'}}>Labour ({totals.lHrs} hrs)</td><td style={{textAlign:'right', padding:'8px'}}>£{totals.lPrice.toFixed(2)}</td></tr><tr style={{borderTop:'2px solid #777'}}><td style={{padding:'8px', textAlign:'right', fontWeight:'bold'}}>Net Total:</td><td style={{textAlign:'right', padding:'8px'}}>£{(totals.total / (1 + (parseFloat(settings.vatRate)/100))).toFixed(2)}</td></tr><tr><td style={{padding:'8px', textAlign:'right'}}>VAT @ {settings.vatRate}%:</td><td style={{textAlign:'right', padding:'8px'}}>£{(totals.total - (totals.total / (1 + (parseFloat(settings.vatRate)/100)))).toFixed(2)}</td></tr><tr style={{borderTop:'2px solid #000'}}><td style={{padding:'8px', textAlign:'right', fontWeight:'bold', fontSize:'14px'}}>TOTAL INC VAT:</td><td style={{textAlign:'right', padding:'8px', fontWeight:'bold', fontSize:'14px'}}>£{totals.total.toFixed(2)}</td></tr></tbody></table>{docType === 'JOB CARD' ? (<div style={{marginTop:'30px', border:'2px dashed #333', padding:'20px'}}><p>{job.repair.techNotes || 'No notes.'}</p></div>) : (<div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'30px'}}><div style={{flex:1, textAlign:'center', border:'4px solid #f97316', borderRadius:'20px', padding:'20px', marginRight:'20px'}}><div style={{fontSize:'20px', fontWeight:'900'}}>BACS: {settings.bank}</div>{settings.paypalQr && <img src={settings.paypalQr} style={{height:'120px', marginTop:'10px'}} alt="QR" />}</div><div style={{flex:1, textAlign:'right'}}><h1 style={{fontSize:'45px', margin:0, color:'#f97316'}}>£{printMode === 'EXCESS' ? parseFloat(job.repair.excess||0).toFixed(2) : printMode === 'INSURER' ? totals.insurer.toFixed(2) : totals.total.toFixed(2)}</h1></div></div>)}</div>
                    )}
                </div>

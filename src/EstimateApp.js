...
const EstimateApp = ({ userId }) => {
    ...
    const resetJob = () => {
        if(window.confirm("⚠️ Clear all fields?")) {
            localStorage.removeItem('mmm_v500_FINAL');
            setJob(INITIAL_JOB);
            setClientMatch(null); 
            window.scrollTo(0, 0); 
        }

...
// --- DESIGN SYSTEM: TITAN TUNGSTEN (V151 ABSOLUTE) ---
...
const EstimateApp = ({ userId }) => {
    ...
    useEffect(() => {
        ...
        const saved = localStorage.getItem('mmm_v151_ABSOLUTE');
    ...
    useEffect(() => { localStorage.setItem('mmm_v151_ABSOLUTE', JSON.stringify(job)); }, [job]);
    ...
    // --- DVLA HANDSHAKE: THE NINTH DEGREE (MIMICS CURL & PYTHON REQUESTS TO THE LETTER) ---
    const runDVLA = async () => {
        ...
        } catch (e) { alert("DVLA Link Refused: Verify Key in Settings."); }
        setLoading(false);
    };
    ...
    return (
        <div style={{ background: '#000', minHeight: '100vh', color: '#fff', padding: '20px', paddingBottom: '180px' }}>
                                ...
                                {history.map((h, i) => (
                                    <div key={i} style={{background:'#000', padding:'25px', borderRadius:'20px', marginBottom:'15px', border:'1px solid #333', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                        ...
                                        <div style={{display:'flex', gap:'10px'}}>
                                            ...
                                            <button style={s.btnG(theme.danger)} onClick={async () => { if(window.confirm("Wipe Repair?")) await deleteDoc(doc(db, 'estimates', h.id)); }}>X</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            {/* PRINT ENGINE: INDUSTRIAL GRID LOCK */}

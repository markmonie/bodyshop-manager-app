...
const EstimateApp = ({ userId }) => {
    ...
    const [settings, setSettings] = useState({ 
        ...
        dvlaKey: 'IXqv1yDD1latEPHIntk2w8MEuz9X57IE9TP9sxGc', logoUrl: '', paypalQr: '', password: '1234', terms: 'Payment due on collection. 12-month repair warranty.' 
    });
    ...
    useEffect(() => {
        ...
        const saved = localStorage.getItem('mmm_active_v20');
    ...
    useEffect(() => { localStorage.setItem('mmm_active_v20', JSON.stringify(job)); }, [job]);
        ...
        <div style={{ background: '#000', minHeight: '100vh', color: '#fff', padding: '20px', paddingBottom: '140px' }}>
            ...
            {view === 'HUB' && (
                <div>
                    ...
                    <div style={s.card(theme.hub)}>
                        ...
                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', fontSize:'12px', opacity:0.8}}>
            ...
            {view === 'EST' && (
                <div>
                    ...
                    <div style={s.card(theme.hub)}>
                        <span style={s.label}>Repair Items Matrix</span>
            ...
            {view === 'WORK' && (
                <div>
                    ...
                    <div style={s.card(theme.work)}>
                        ...
                        <span style={s.label}>Technician Report</span>
                ...
                <div>
                    ...
                    <div style={s.card(theme.fin)}>
                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'25px'}}>
                            <div style={{background:'#000', padding:'25px', borderRadius:'15px'}}><span style={s.label}>Total Income</span><h2 style={{margin:0}}>£{history.reduce((a,b)=>a+(b.totals?.total||0),0).toFixed(2)}</h2></div>
                        ...
                        <span style={s.label}>Expenditure Upload</span>
                        <input type="file" onChange={(e) => handleFileUpload(e, 'finances', 'expenses')} style={{marginBottom:'15px'}} />
                        <button style={{...s.btnG(theme.fin), width:'100%'}} onClick={() => {
                            let csv = "Date,Reg,Client,Total\n";
                            history.forEach(h => csv += `${new Date(h.createdAt?.seconds*1000).toLocaleDateString()},${h.vehicle?.reg},${h.client?.name},${h.totals?.total}\n`);
                        ...
                        <div key={i} style={s.card('#444')}>
                            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                ...
                                <div style={{display:'flex', gap:'10px'}}>
                                    ...
                                    <button style={{...s.btnG(theme.danger), padding:'12px 20px'}} onClick={async () => { if(window.confirm("Delete?")) await deleteDoc(doc(db, 'estimates', h.id)); }}>X</button>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {view === 'SET' && (
                <div>
                    ...
                    <div style={s.card(theme.set)}>
                        <span style={s.label}>Logo & PayPal QR (Files)</span>
                        <input type="file" onChange={(e) => handleFileUpload(e, 'branding', 'logoUrl')} />
                        <input type="file" onChange={(e) => handleFileUpload(e, 'branding', 'paypalQr')} style={{marginTop:'10px'}} />
                        <hr style={{borderColor:'#333', margin:'20px 0'}}/>
                        ...
                        <button style={{...s.btnG(theme.set), width:'100%', marginTop:'10px'}} onClick={async () => { await setDoc(doc(db, 'settings', 'global'), settings); alert("Saved"); }}>SAVE CONFIG</button>
                    </div>
                </div>
            )}
            ...
            <div className="print-only" style={{display:'none', color:'black', padding:'60px', fontFamily:'Arial'}}>
                <div style={{display:'flex', justifyContent:'space-between', borderBottom:'6px solid #f97316', paddingBottom:'30px'}}>
                    <div>{settings.logoUrl && <img src={settings.logoUrl} style={{height:'110px'}} />}<h1 style={{margin:0, color:'#f97316'}}>{settings.coName}</h1><p>{settings.address}</p></div>
                    <div style={{textAlign:'right'}}><h2 style={{color:'#f97316', fontSize:'48px', margin:0}}>INVOICE</h2><p>Date: {new Date().toLocaleDateString()}<br/>Reg: {job.vehicle.reg}</p></div>
                </div>
                <div style={{marginTop:'30px', fontWeight:'bold'}}>Technician Report:</div>
                <div style={{minHeight:'80px', border:'1px solid #eee', padding:'15px', marginTop:'10px'}}>{job.repair.techNotes || 'Standard repair procedure.'}</div>
                ...
                <div style={{display:'flex', justifyContent:'space-between', marginTop:'45px'}}>
                    <div>{settings.paypalQr && <img src={settings.paypalQr} style={{height:'140px'}} />}</div>
                    <div style={{width:'50%', textAlign:'right'}}>
                        <h1 style={{color:'#f97316', fontSize:'52px'}}>£{totals.total.toFixed(2)}</h1>
                        <div style={{background:'#fff3e0', padding:'30px', border:'3px solid #f97316', borderRadius:'20px', textAlign:'left'}}>
                            <div style={{display:'flex', justifyContent:'space-between', marginBottom:'12px'}}><span>CLIENT EXCESS:</span><strong>£{totals.customer.toFixed(2)}</strong></div>
                ...
                <div style={{marginTop:'100px', borderTop:'1px solid #eee', paddingTop:'30px'}}>Bank: {settings.bank} | Warranty: {settings.terms}</div>
            </div>

...
// --- DESIGN SYSTEM: TITAN TUNGSTEN (V160 ABSOLUTE) ---
...
const EstimateApp = ({ userId }) => {
    ...
    useEffect(() => {
        ...
        const saved = localStorage.getItem('mmm_v160_TITANIUM');
    ...
    useEffect(() => { localStorage.setItem('mmm_v160_TITANIUM', JSON.stringify(job)); }, [job]);
    ...
    // --- NINTH DEGREE DVLA HANDSHAKE (MIMICS YOUR PYTHON EXACTLY) ---
    const runDVLA = async () => {
        ...
        } catch (e) { alert("DVLA Refused: Check API Key or Registry."); }
        setLoading(false);
    };
    ...
    return (
        <div style={{ background: '#000', minHeight: '100vh', color: '#fff', padding: '20px', paddingBottom: '180px' }}>
                ...
                {view === 'HUB' && (
                    <div style={{maxWidth:'850px', margin:'0 auto'}}>
                        ...
                        <div style={s.card(theme.hub)}>
                            ...
                            {job.lastSuccess && <div style={{background:theme.deal, color:'#000', padding:'10px', borderRadius:'10px', fontSize:'12px', textAlign:'center', marginBottom:'15px', fontWeight:'bold'}}>DVLA AUDIT: DATA VERIFIED AT {job.lastSuccess}</div>}
                        ...
                        <div style={s.card(theme.deal)}>
                            <span style={s.label}>Station 3: CRM Stakeholders</span>
                            <div style={{background:'#000', padding:'35px', borderRadius:'25px', border:'2px solid #333', height:'350px', overflowY:'auto'}}>
                                {addressBook.map((c, i) => (
                                    <div key={i} style={{borderBottom:'1px solid #222', padding:'20px 0', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                        ...
                                        <button style={{...s.btnG(theme.deal), padding:'12px 20px', fontSize:'14px'}} onClick={() => setJob({...job, client: { name: c.name, phone: c.phone, email: c.email, address: c.address }})}>+ ASSIGN</button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div style={s.card(theme.work)}>
                            <span style={s.label}>Station 4: AI Intelligent Estimating</span>
                            <div style={s.displayBox}><span style={s.label}>AI READY DATE</span><div style={{color:theme.hub, fontSize:'45px', fontWeight:'900'}}>{projectedDate}</div></div>
                    ...
                    <div style={{maxWidth:'850px', margin:'0 auto'}}>
                        ...
                        <div style={s.card(theme.deal)}>
                            ...
                            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginTop:'20px'}}>
                                <button style={s.btnG('#333')} onClick={() => { setDocType('ESTIMATE'); setTimeout(() => window.print(), 100); }}>ESTIMATE</button>
                                <button style={s.btnG(theme.deal)} onClick={() => { setDocType('INVOICE'); setTimeout(() => window.print(), 100); }}>INVOICE</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {view === 'SET' && (
                    <div style={{maxWidth:'850px', margin:'0 auto'}}>
                        ...
                        <div style={s.card(theme.set)}>
                            ...
                            <button style={{...s.btnG(theme.deal), width:'100%', padding:'35px'}} onClick={async () => { await setDoc(doc(db, 'settings', 'global'), settings); alert("Locked."); }}>SAVE GLOBAL SETTINGS</button>
                        </div>
                    </div>
                )}
            ...
            {/* PRINT ENGINE: INDUSTRIAL GRID LOCK (ZERO OVERLAP) */}
            <div className="print-only" style={{display:'none', color:'black', padding:'50px', fontFamily:'Arial', width:'100%', boxSizing:'border-box'}}>
                ...
                <div style={{display:'grid', gridTemplateColumns:'1.2fr 1fr', gap:'40px', marginTop:'60px'}}>
                    <div style={{textAlign:'center', border:'2px solid #f97316', borderRadius:'25px', padding:'25px'}}>
                        ...
                        {job.vault?.signature && <img src={job.vault.signature} style={{width:'250px', marginTop:'20px'}} />}
                    </div>
